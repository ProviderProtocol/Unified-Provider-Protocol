...
/**
 * Execute fetch with retry, timeout, error normalization, and custom headers.
 * Custom headers from config.headers are merged with init.headers.
 */
...
<Aside type="note">
Capabilities reflect what the **provider's API** supports, not individual model capabilities. If a user attempts to use a feature with a model that doesn't support it, the provider's API will return an error—this is expected behavior.
</Aside>
