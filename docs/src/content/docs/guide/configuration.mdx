...
```typescript
import { RoundRobinKeys } from '@providerprotocol/ai/http';
...
```typescript
import { WeightedKeys } from '@providerprotocol/ai/http';
...
```typescript
import { DynamicKey } from '@providerprotocol/ai/http';
...
```
  ...
  <TabItem label="OpenRouter Attribution">
```typescript
const model = llm({
...
```
  ...
  <TabItem label="OpenAI Organization">
```typescript
const gpt = llm({
  model: openai('gpt-4o'),
  config: {
...
<Aside type="note">
Custom headers are merged with provider-required headers (e.g., `Content-Type`, `Authorization`). If a custom header conflicts with a required header, the custom header takes precedence, allowing you to override default behavior when necessary.
</Aside>
...
import { RoundRobinKeys, ExponentialBackoff } from '@providerprotocol/ai/http';
