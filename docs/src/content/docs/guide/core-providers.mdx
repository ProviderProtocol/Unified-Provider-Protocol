...
UPP supports ten AI providers out of the box. Each provider is a separate import that can be used with `llm()`, `embedding()`, or `image()`.
...
| **Groq** | ✓ | | | Fast inference with Llama, Gemma, Mixtral |
| **Cerebras** | ✓ | | | Ultra-fast inference with Llama, Qwen |
| **Responses** | ✓ | | | OpenResponses-compatible endpoints |
...
const turn = await searchModel.generate('What are the latest AI news today?');
```

### With RAG Documents

Provide context documents with optional citations:
...
const ragModel = llm({
  ...
  params: {
    documents: [
      { title: 'Company Policy', content: 'Employees get 20 vacation days...' },
      { title: 'Benefits Guide', content: 'Health insurance covers...' },
    ],
    citation_options: { include: true },
  },
});

const turn = await ragModel.generate('How many vacation days do employees get?');
```
...
**Popular models:** `llama-3.3-70b`, `llama-3.1-8b`, `qwen-3-32b`, `gpt-oss-120b`

## Responses

Connect to any OpenResponses-compatible endpoint. This provider implements the OpenResponses specification for interoperable LLM interfaces.

```typescript
import { llm } from '@providerprotocol/ai';
import { responses } from '@providerprotocol/ai/responses';

// Connect to OpenAI using the OpenResponses spec
const model = llm({
...
interface ResponsesProviderOptions {
  host: string;        // Required: Base URL of the OpenResponses server
...
The responses provider works with any OpenResponses-compatible endpoint:

```typescript
// OpenAI
responses('gpt-4o', { host: 'https://api.openai.com/v1', apiKeyEnv: 'OPENAI_API_KEY' })

// OpenRouter
responses('anthropic/claude-sonnet-4', { host: 'https://openrouter.ai/api/v1', apiKeyEnv: 'OPENROUTER_API_KEY' })

// Self-hosted server
responses('my-model', { host: 'https://my-server.example.com/v1' })
```

<Aside type="tip">
Use the responses provider when you want a standardized interface that works across multiple OpenResponses-compatible services without changing your code.
</Aside>
...
| Groq | `GroqLLMParams` | - | - |
| Cerebras | `CerebrasLLMParams` | - | - |
| Responses | `ResponsesParams` | - | - |
...
GROQ_API_KEY=...
CEREBRAS_API_KEY=...
OPENRESPONSES_API_KEY=...  # For responses provider (or use apiKeyEnv option)
```
