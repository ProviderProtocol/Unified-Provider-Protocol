...
- **Added** `tool_execution_start` streaming event emitted when tool execution begins
- **Added** `tool_execution_end` streaming event emitted when tool execution completes
- **Added** `result`, `isError`, and `timestamp` fields to `EventDelta` for tool execution events

### Provider-Native Tools

- **Added** Provider-Native Tools section documenting server-side built-in tools (web search, image generation, code interpreter, etc.)
- **Added** guidance for passing provider-native tools via `params.tools` pass-through
- **Added** output handling patterns for transforming native tool results to standard content blocks

### Message Accessors

- **Added** `images`, `audio`, `video` convenience accessors to base Message structure to match existing `text` accessor pattern
