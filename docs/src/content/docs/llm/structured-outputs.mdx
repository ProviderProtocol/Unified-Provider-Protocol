...
<Aside type="note">
If a provider doesn't natively support structured outputs, it MUST:
1. Attempt to get JSON output
2. Validate the response against the schema
3. Throw `UPPError` with code `INVALID_RESPONSE` if validation fails
</Aside>
